...
void main() {
  ...
  float s = abs(sin(PI2*(u_time -d)/animationSeconds)/2.0 - d);
  px = rotate2D(px, radians(4.0 * 180. * s * d + d2));
  px = px * 2.0 - 1. -d;
  color += fract(s * s);
  s = s * 0.5;
  color += vec3(circle(px, 0.3, vec2(s, 0.5)));
  color += vec3(circle(px, 0.3, vec2(s - 0.5, -0)));
  color += vec3(circle(px, 0.3 - px.x * 0., vec2(s)));
  color += vec3(circle(px, 0.3 - px.y * 0., vec2(s, -0.5)));
  color += vec3(circle(px, 0.3 - px.x * 0., vec2(0.5, s)));
  color *= vec3(fract(d * px.x - px.y), 0.0, fract(px.x + px.y));
