...
void main(void)
{
    ...
    for (int i=0;i<10;++i) {
      ...
      float circle1Radius = idx * 0.1+(1.0-sineVal * idx * 0.3 - 0.3)*0.2; //radius of circle
      vec2 circle1Center = mix(vec2(-0.0,0.01 * idx * 0.3 ),vec2(0.58,0.0),sineVal * idx * 0.1);
      
      float circle2Radius = idx * 0.1+(sineVal * idx * 0.3 )*0.2; //radius of circle
      vec2 circle2Center = mix(vec2(0.0,-0.5 * idx * 0.3 ),vec2(0.0,0.8 * idx * 0.1 ),cosVal * idx * 0.1);
      
      float circle3Radius = idx * 0.1+(idx * 0.5-sineVal * idx * 0.21 ) * 0.2; //radius of circle
      vec2 circle3Center = mix(vec2(-0.5 * idx * 0.1 ,-0.55),vec2(0.58 * idx * 0.1 ,0.8),sineVal * idx * 0.1);
      
      float circle4Radius = idx * 0.1+(idx * 0.5-sineVal * idx * 0.21 ) * 0.2; //radius of circle
      ...
      if (length(uv-circle2Center)<circle2Radius) {
          color += vec4(0,0.0,normalizedIdx * 0.2,normalizedIdx * 0.2);
      } 
