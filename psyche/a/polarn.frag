...
float random(in vec2 _st){
    return fract(sin(dot(_st.xy,
        ...
        void main(){
            ...
            if(mod(ipos.y,2.)==0.){
                //px=rotate2D(px,u_time*.075*PI2);
            }
            
            // get  vector in the center;
            vec2 ctr=vec2(.5)-px;
            for(int i=0;i<1;i++){
                ctr.xy=abs(ctr)/abs(dot(ctr+s-.15,abs(ctr-s+.25)))-vec2(.5+s);
                //px.xy=abs(px)/abs(dot(px,ctr))-vec2(.5*s);
            ...
            vec3 color=vec3(px.x*s-.75,px.y*.01,abs(px.x*px.y*s));
            vec2 tile=truchetPattern(fpos,random(ipos));
            float tileAngle=atan(tile.y,tile.x);
            
            // get  distance from the center;
            
            float radius=length(ctr)-1.5*s;
            float angle=atan(ctr.y,ctr.x)*6.*s;
            float f=cos(angle*4.);
            float cs=cos(angle*4.);
            
            //color+=vec3(smoothstep(f,f-f*.02,radius));
            //color+=step(ctr.x,fract(mod(ctr.y,10.)));
            color+=vec3(smoothstep(f,f-s,radius)-.1,smoothstep(f,f-s,radius)*.01,smoothstep(f,f-s,radius)-s+.2);
