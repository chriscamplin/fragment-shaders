    ...
    return fract(sin(dot(_st.xy,
        ...
        void main(){
            ...
            for(int i=0;i<2;i++){
                ...
                ctr.xy=abs(ctr)/abs(dot(ctr+s-.75,ctr-s+.75))-vec2(.5*fract(s));
            }
            vec3 color=vec3(px.x*s,px.y*.1,abs(px.x*px.y));
            vec2 tile=truchetPattern(fpos,random(ipos));
            // get  distance from the center;
            float tileAngle=atan(tile.y,tile.x);
            
            float radius=length(ctr)*u_mouse.x;
            float angle=atan(ctr.y,ctr.x)*.4;
            float f=cos(angle*10.);
