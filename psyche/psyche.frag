...
//p = fract(p);
for(int i=0;i<4;++i){
    p.yx=abs(p)/abs(dot(p.xy,p.xy))-vec2(s, c);//.9+cos(time*.2)*.4);
...
//newColor+=abs(cross(pastelBlue,newColor));
...
//newColor.g=.01;
newColor = mix(color, newColor, p.x);
newColor = hsb2rgb(newColor);
newColor.r=abs(newColor.r)-s;
newColor.g=0.0;//abs(s)-1.;
newColor.b=0.1;//abs(c);
newColor = normalize(newColor);
