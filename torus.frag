...
void main() {
    ...
    uv=rotate2D(uv, PI2*u_time*0.1)*.333;
    //setup camera at pos 0
    //ray_origin
    vec3 ro=vec3(0., 2.1275, -1.);
    vec3 lookat=vec3(0.);
    float zoom = .25;
    float t = u_time;
    // forward
    ...
    vec3 p;
    float centerRadius = .99999999;
    for(int i = 0; i< 100; i++) { 
        p = ro + rd * distOrigin;
        distSurface = (length(vec2(length(p.xz)-1.0175, p.y))-centerRadius);
   ...
   // col = rd;
    if(distSurface <.001) {
        ...
        float x=atan(p.x,p.z); // atan returns -pi t pi
        float y=atan(length(p.xzy)-1.,p.y);
        float bands = sin(y*30.+x*6.);
